...
export default function VolumeSelect({
	...
	const previousVolume = (Number.parseInt(currentVolumeFromUrl) - 1).toString();
	const nextVolume = (Number.parseInt(currentVolumeFromUrl) + 1).toString();
		...
		<div className={`flex flex-wrap items-center justify-center ${className}`}>
			{currentVolume && (
				<div className="flex justify-center space-x-2">
					{Number.parseInt(previousVolume) > 0 ? (
			...
			<div className="flex flex-wrap justify-center mt-2 w-full">
				...
				>
					...
					<SelectContent>
						{volumes
							.sort((a, b) => {
								const volumeANumber = Number.parseInt(formatVolume(a.name));
								const volumeBNumber = Number.parseInt(formatVolume(b.name));
